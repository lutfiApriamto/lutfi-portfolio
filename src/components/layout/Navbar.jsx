...
const socialLinks = [
  {
    ...
    hoverColorDark: "#ffffff", hoverColorLight: "#171515",
    glowDark: "rgba(255,255,255,0.3)", glowLight: "rgba(23,21,21,0.2)",
  },
  {
    name: "linkedin", icon: FiLinkedin,
    url: "https://www.linkedin.com/in/lutfi-apriamto-3a9383312/",
    hoverColorDark: "#0A66C2", hoverColorLight: "#0A66C2",
    glowDark: "rgba(10,102,194,0.4)", glowLight: "rgba(10,102,194,0.3)",
  },
  {
    name: "instagram", icon: FiInstagram,
    url: "https://www.instagram.com/lutfiamto/",
    hoverColorDark: "#E1306C", hoverColorLight: "#E1306C",
    glowDark: "rgba(225,48,108,0.4)", glowLight: "rgba(225,48,108,0.3)",
  },
];
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ background: isDark ? "#ffffff" : "#050505" }}
      />
      <motion.div
        animate={{ color: hovered ? (isDark ? "#050505" : "#ffffff") : (isDark ? "#ffffff" : "#050505") }}
...
const HamburgerButton = ({ isOpen, onClick, isDark }) => (
  <ExpandButton onClick={onClick} isDark={isDark} ariaLabel="Toggle Menu">
    <div className="flex flex-col items-center justify-center gap-[5px] w-full h-full relative">
      <motion.span
        animate={{ y: isOpen ? 7 : 0, rotate: isOpen ? 45 : 0, width: isOpen ? "22px" : "24px" }}
        transition={{ duration: 0.5, ease: premiumEase }}
        className="h-0.5 rounded-full bg-current block"
        style={{ width: "24px" }}
      />
      <motion.span
...
const BrandLogo = ({ isDark }) => (
  <Link to="/" className="hidden md:block">
    <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.97 }} className="group relative cursor-pointer select-none">
      <span className="text-lg md:text-xl font-black tracking-[0.35em]" style={{ color: isDark ? "#ffffff" : "#050505" }} />
      <motion.div
        className="absolute left-0 -bottom-1 h-[2px] w-full origin-left"
        style={{ backgroundColor: isDark ? "#ffffff" : "#050505" }}
...
const FullscreenMenu = ({ isOpen, onClose, isDark }) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          // z-[9990] — di bawah Navbar (z-[9999]) dan JAUH di bawah Modal (z-[10000])
        ...
        >
          <nav className="flex flex-col items-center justify-center gap-4 md:gap-6 text-center select-none w-full">
            {menuItems.map((item, i) => {
              ...
              const itemOpacity      = isAnyHovered
                ? (isThisHovered ? 1 : 0.25)
                : (!isActive ? 0.4 : 1);
              ...
              return (
                <div key={item.label} className="relative py-2 w-full max-w-2xl px-4">
                  ...
                  >
                    ...
                    <motion.span
                      initial={{ scaleX: 0 }}
                      animate={{ scaleX: isThisHovered ? 1 : 0 }}
                      transition={{ duration: 0.4, ease: premiumEase }}
                      className="absolute left-0 right-0 top-[52%] h-[3px] bg-current origin-left pointer-events-none"
                    />
                  </Link>
                </div>
              );
          ...
          <div className="absolute bottom-8 left-6 right-6 md:bottom-10 md:left-16 md:right-16 flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-2 text-[10px] md:text-[11px] font-medium tracking-[0.2em] uppercase opacity-60">
            ...
            <div className="flex items-center gap-5">
              {socialLinks.map((social, index) => {
                ...
                const accent    = isHovered ? (isDark ? social.hoverColorDark : social.hoverColorLight) : "currentColor";
                return (
                  <motion.a
                    key={index}
                    href={social.url} target="_blank" rel="noopener noreferrer"
                    className="p-2 rounded-full border border-transparent flex items-center justify-center"
                    ...
                    animate={{
                      ...
                      boxShadow: isHovered ? `0 4px 15px ${isDark ? social.glowDark : social.glowLight}` : "none",
...
const Navbar = ({ wasIntroShown }) => {
  ...
  const bounceTransition = { type: "spring", damping: 11, stiffness: 120, mass: 1 };
  const delayOffset      = wasIntroShown ? 0.45 : 0;

  return (
    <>
      {/*
        z-[9999] — di atas konten biasa & FullscreenMenu (z-[9990])
        TAPI di bawah Modal (z-[10000])
        Jadi: Modal (10000) > Navbar (9999) > FullscreenMenu (9990) > konten biasa
      */}
      <div 
          className="fixed top-0 left-0 right-0 z-[9999] p-4 md:p-6 flex justify-between items-center pointer-events-none"
          style={{
